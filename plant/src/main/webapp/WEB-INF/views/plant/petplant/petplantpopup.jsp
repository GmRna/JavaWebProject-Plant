		      ...
		      <div class="feeds" id="petplant" >
		        <!-- article -->
			        <article  id="petplantDiv">
			          <header>
			            <div class="profile-of-article">
			              ...
			              작성자 : ${petboard.user_nick} 
			              로그인 회원 번호 : ${loginInfo.user_no} 
			              작성자 회원 번호 : ${petboard.user_writeNo }</span>
			            </div>
			            <!-- ... 아이콘 -->
			            <!-- 더보기 버튼  -->
				    ...
				    <form action="" method="post" id="replyfrm">
					   	<div class="comment">
				       		<span class="check" >
				       		<input type="checkbox" name="ppr_secretCheck" id="check" value="on"> <label for="check" >비밀댓글</label></span>
					       	<input id="input-comment2" name="ppr_content" class="input-comment" type="text" placeholder="댓글 달기..." >
					       	<input type="hidden" name="pet_no" value="${petboard.pet_no }">
					       	<c:if test="${empty loginInfo}">
					      	 	<button type="button" class="submit-comment" onclick="addreplylogin()" disabled>게시</button>
					       	</c:if>
					       	<c:if test="${!empty loginInfo}">
